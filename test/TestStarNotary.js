...
it('can add the star name and star symbol properly', async() => {
    ...
    const name = "UdacityCourse";
    const symbol = "IND"
    // assert statement for name
    assert.equal(await instance.name.call(), "UdacityCourse");
    // assert statement for symbol 
    assert.equal(await instance.symbol.call(), "IND");
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 40;
    await instance.createStar('Astar', starId, { from: user1 });

    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);

    // 3. Verify if you Star name is the same
    assert.equal(starName, "Astar");
});